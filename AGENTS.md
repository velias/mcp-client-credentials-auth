...
- **Local disconnect cleanup** -- stdio: detects `localServer.onclose` (stdin closed), awaits remote close with 2s timeout, then exits. HTTP: transport `onclose` always tears down that session (including half-init before `sessionId` is assigned); no `process.exit`

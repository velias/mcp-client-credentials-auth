...
Sessions with no inbound MCP traffic for `MCP_CC_PROXY_HTTP_SESSION_IDLE_SECONDS` (default 30 minutes; `0` disables) are closed and removed. That cleans up clients that abandon a session without `DELETE` (for example some MCP reload/logout paths). A later request that still sends the old `mcp-session-id` receives `404` / `Session not found`; the proxy does **not** silently attach that id to a new session. Spec-compliant Streamable HTTP clients re-`initialize` without a session id and continue on a new session.
...
| Remote server needs no OAuth (`no-auth`) | Token not required; remote MCP must still be reachable (or fail-fast on HTTP 4xx except 408/429). Missing `.well-known` metadata looks like `no-auth`; a typo in the MCP path then fails immediately at Phase 1 rather than retrying. |
| Remote MCP URL returns HTTP 4xx at startup (except 408/429) | Immediate exit; no startup retries and no SSE fallback. Check `MCP_CC_PROXY_REMOTE_MCP_URL` (wrong path is the usual cause). Stderr includes the HTTP status and a truncated response body. 408 and 429 are retried as transient. |
